...
-- logic for flag_quote

Alter table reporting.combined_policy_data add flag_quote_1 int 
default(0),
    flag_quote_7 int default(0),
    flag_quote_30 int default(0),
    flag_quote_90 int default(0);

update reporting.combined_policy_data e1
set e1.flag_quote_1 = 1
from (select 
      oid,
      floor(datediff(day, created_date::date, current_date()::date)/1) as 
nday_1,
      RANK() OVER (PARTITION BY person_birthdate, person_address_postcode, 
object_vehicle_hsn, object_vehicle_tsn, nday_1 ORDER BY flag_converted 
DESC, created_date desc) as ranking_1
      from reporting.combined_policy_data
      where product_id in ('p_CarClassicGreenCombined', 
'p_CarNewGermany','p_CarPowerTariff','p_YsMwxuaCF9RAcnDXTvkGwERF','p_YsMwxuaCF9RAcnDXTvkGwSWT')
     order by person_birthdate, person_address_postcode, 
object_vehicle_hsn, object_vehicle_tsn, flag_converted desc, created_date 
desc) e2
where e2.ranking_1 = 1 and e1.oid = e2.oid;
...
product_private_liability_policyholder_birthdate, 
product_private_liability_number_insured_children, 
product_private_liability_number_insured_adults, nday_1 ORDER BY 
flag_converted DESC, created_date desc) as ranking_1
      from reporting.combined_policy_data
      where product_id in ('p_PrivateLiability', 
'p_YsMwxuaCF9RAcnDXTvkGcwsssdPISW')
     ) e2
where e2.ranking_1 = 1 and e1.oid = e2.oid;
...
from (select 
      ...
      floor(datediff(day, created_date::date, current_date()::date)/1) as 
nday_1,
      RANK() OVER (PARTITION BY object_structure_home_size, 
product_household_policyholder_birthdate, 
object_structure_home_address_postcode, nday_1 ORDER BY flag_converted 
DESC, created_date desc) as ranking_1
      from reporting.combined_policy_data
      where product_id in ('p_Household', 'p_YsMwxuaCF9RAcnDXTvkGcwPUSW')
     ) e2
where e2.ranking_1 = 1 and e1.oid = e2.oid;
...
nday_1,
      RANK() OVER (PARTITION BY 
object_vehicle_driver_nationality,object_vehicle_driver_birthdate,object_vehicle_eurotax_id,object_vehicle_driver_address_postcode, 
nday_1 ORDER BY flag_converted DESC, created_date desc) as ranking_1
      from reporting.combined_policy_data
      where product_id in 
('p_CarCSuiteSwitzerland','p_CarSwitchSwitzerland')
     ) e2
where e2.ranking_1 = 1 and e1.oid = e2.oid;
...
from (select 
      ...
      floor(datediff(day, created_date::date, current_date()::date)/1) as 
nday_1,
      RANK() OVER (PARTITION BY product_home_policyholder_birthdate, 
product_home_number_of_adults,product_home_number_of_children,object_structure_home_address_postal_code, 
product_home_policyholder_nationality , nday_1 ORDER BY flag_converted 
DESC, created_date desc) as ranking_1
      from reporting.combined_policy_data
      where product_id in ('p_HomeCSuiteSwitzerland')
     ) e2
where e2.ranking_1 = 1 and e1.oid = e2.oid;
...
update reporting.combined_policy_data e1
set e1.flag_quote_7 = 1
from (select 
      oid,
      floor(datediff(day, created_date::date, current_date()::date)/7) as 
nday_7,
      RANK() OVER (PARTITION BY person_birthdate, person_address_postcode, 
object_vehicle_hsn, object_vehicle_tsn, nday_7 ORDER BY flag_converted 
DESC, created_date desc) as ranking_7
      from reporting.combined_policy_data
      where product_id in ('p_CarClassicGreenCombined', 
'p_CarNewGermany','p_CarPowerTariff','p_YsMwxuaCF9RAcnDXTvkGwERF','p_YsMwxuaCF9RAcnDXTvkGwSWT')
     order by person_birthdate, person_address_postcode, 
object_vehicle_hsn, object_vehicle_tsn, flag_converted desc, created_date 
desc) e2
where e2.ranking_7 = 1 and e1.oid = e2.oid;
...
product_private_liability_policyholder_birthdate, 
product_private_liability_number_insured_children, 
product_private_liability_number_insured_adults, nday_7 ORDER BY 
flag_converted DESC, created_date desc) as ranking_7
      from reporting.combined_policy_data
      where product_id in ('p_PrivateLiability', 
'p_YsMwxuaCF9RAcnDXTvkGcwsssdPISW')
     ) e2
...
nday_7,
      RANK() OVER (PARTITION BY object_structure_home_size, 
product_household_policyholder_birthdate, 
object_structure_home_address_postcode, nday_7 ORDER BY flag_converted 
DESC, created_date desc) as ranking_7
      from reporting.combined_policy_data
      where product_id in ('p_Household', 'p_YsMwxuaCF9RAcnDXTvkGcwPUSW')
     ) e2
...
nday_7,
      RANK() OVER (PARTITION BY 
object_vehicle_driver_nationality,object_vehicle_driver_birthdate,object_vehicle_eurotax_id,object_vehicle_driver_address_postcode, 
nday_7 ORDER BY flag_converted DESC, created_date desc) as ranking_7
      from reporting.combined_policy_data
      where product_id in 
('p_CarCSuiteSwitzerland','p_CarSwitchSwitzerland')
     ) e2
...
nday_7,
      RANK() OVER (PARTITION BY product_home_policyholder_birthdate, 
product_home_number_of_adults,product_home_number_of_children,object_structure_home_address_postal_code, 
product_home_policyholder_nationality , nday_7 ORDER BY flag_converted 
DESC, created_date desc) as ranking_7
      from reporting.combined_policy_data
      where product_id in ('p_HomeCSuiteSwitzerland')
     ) e2
...
update reporting.combined_policy_data e1
set e1.flag_quote_30 = 1
from (select 
      oid,
      floor(datediff(day, created_date::date, current_date()::date)/30) as 
nday_30,
      RANK() OVER (PARTITION BY person_birthdate, person_address_postcode, 
object_vehicle_hsn, object_vehicle_tsn, nday_30 ORDER BY flag_converted 
DESC, created_date desc) as ranking_30
      from reporting.combined_policy_data
      where product_id in ('p_CarClassicGreenCombined', 
'p_CarNewGermany','p_CarPowerTariff','p_YsMwxuaCF9RAcnDXTvkGwERF','p_YsMwxuaCF9RAcnDXTvkGwSWT')
     order by person_birthdate, person_address_postcode, 
object_vehicle_hsn, object_vehicle_tsn, flag_converted desc, created_date 
desc) e2
where e2.ranking_30 = 1 and e1.oid = e2.oid;
...
product_private_liability_policyholder_birthdate, 
product_private_liability_number_insured_children, 
product_private_liability_number_insured_adults, nday_30 ORDER BY 
flag_converted DESC, created_date desc) as ranking_30
      from reporting.combined_policy_data
      where product_id in ('p_PrivateLiability', 
'p_YsMwxuaCF9RAcnDXTvkGcwsssdPISW')
     ) e2
...
nday_30,
      RANK() OVER (PARTITION BY object_structure_home_size, 
product_household_policyholder_birthdate, 
object_structure_home_address_postcode, nday_30 ORDER BY flag_converted 
DESC, created_date desc) as ranking_30
      from reporting.combined_policy_data
      where product_id in ('p_Household', 'p_YsMwxuaCF9RAcnDXTvkGcwPUSW')
     ) e2
...
nday_30,
      RANK() OVER (PARTITION BY 
object_vehicle_driver_nationality,object_vehicle_driver_birthdate,object_vehicle_eurotax_id,object_vehicle_driver_address_postcode, 
nday_30 ORDER BY flag_converted DESC, created_date desc) as ranking_30
      from reporting.combined_policy_data
      where product_id in 
('p_CarCSuiteSwitzerland','p_CarSwitchSwitzerland')
     ) e2
...
nday_30,
      RANK() OVER (PARTITION BY product_home_policyholder_birthdate, 
product_home_number_of_adults,product_home_number_of_children,object_structure_home_address_postal_code, 
product_home_policyholder_nationality , nday_30 ORDER BY flag_converted 
DESC, created_date desc) as ranking_30
      from reporting.combined_policy_data
      where product_id in ('p_HomeCSuiteSwitzerland')
     ) e2
...
update reporting.combined_policy_data e1
set e1.flag_quote_90 = 1
from (select 
      oid,
      floor(datediff(day, created_date::date, current_date()::date)/90) as 
nday_90,
      RANK() OVER (PARTITION BY person_birthdate, person_address_postcode, 
object_vehicle_hsn, object_vehicle_tsn, nday_90 ORDER BY flag_converted 
DESC, created_date desc) as ranking_90
      from reporting.combined_policy_data
      where product_id in ('p_CarClassicGreenCombined', 
'p_CarNewGermany','p_CarPowerTariff','p_YsMwxuaCF9RAcnDXTvkGwERF','p_YsMwxuaCF9RAcnDXTvkGwSWT')
     order by person_birthdate, person_address_postcode, 
object_vehicle_hsn, object_vehicle_tsn, flag_converted desc, created_date 
desc) e2
where e2.ranking_90 = 1 and e1.oid = e2.oid;
...
product_private_liability_policyholder_birthdate, 
product_private_liability_number_insured_children, 
product_private_liability_number_insured_adults, nday_90 ORDER BY 
flag_converted DESC, created_date desc) as ranking_90
      from reporting.combined_policy_data
      where product_id in ('p_PrivateLiability', 
'p_YsMwxuaCF9RAcnDXTvkGcwsssdPISW')
     ) e2
...
nday_90,
      RANK() OVER (PARTITION BY object_structure_home_size, 
product_household_policyholder_birthdate, 
object_structure_home_address_postcode, nday_90 ORDER BY flag_converted 
DESC, created_date desc) as ranking_90
      from reporting.combined_policy_data
      where product_id in ('p_Household', 'p_YsMwxuaCF9RAcnDXTvkGcwPUSW')
     ) e2
...
nday_90,
      RANK() OVER (PARTITION BY 
object_vehicle_driver_nationality,object_vehicle_driver_birthdate,object_vehicle_eurotax_id,object_vehicle_driver_address_postcode, 
nday_90 ORDER BY flag_converted DESC, created_date desc) as ranking_90
      from reporting.combined_policy_data
      where product_id in 
('p_CarCSuiteSwitzerland','p_CarSwitchSwitzerland')
     ) e2
...
nday_90,
      RANK() OVER (PARTITION BY product_home_policyholder_birthdate, 
product_home_number_of_adults,product_home_number_of_children,object_structure_home_address_postal_code, 
product_home_policyholder_nationality , nday_90 ORDER BY flag_converted 
DESC, created_date desc) as ranking_90
      from reporting.combined_policy_data
      where product_id in ('p_HomeCSuiteSwitzerland')
     ) e2
